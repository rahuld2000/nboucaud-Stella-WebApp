...
export class UnreadsStatusHandlerClass extends React.PureComponent<Props> {
    ...
    updateTitle = () => {
        ...
        const mentionTitle =
            unreadMentionCount > 0 ? `(${unreadMentionCount}) ` : "";
        const unreadTitle =
            !this.isDynamicFaviconSupported && isUnread ? "* " : "";
    ...
    updateFavicon = (badgeStatus: BadgeStatus) => {
        ...
        const link16x16 = document.querySelector<HTMLLinkElement>(
            'link[rel="icon"][sizes="16x16"]'
        );
        const link24x24 = document.querySelector<HTMLLinkElement>(
            'link[rel="icon"][sizes="24x24"]'
        );
        const link32x32 = document.querySelector<HTMLLinkElement>(
            'link[rel="icon"][sizes="32x32"]'
        );
        const link64x64 = document.querySelector<HTMLLinkElement>(
            'link[rel="icon"][sizes="64x64"]'
        );
        const link96x96 = document.querySelector<HTMLLinkElement>(
